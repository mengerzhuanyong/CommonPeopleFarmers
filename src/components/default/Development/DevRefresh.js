...
function DevRefresh() {
      ...
      actions: [
        {
          ...
          onPress: () => {
            ActionManager.hide();
            RouterHelper.navigate('Demo示例', 'Example');
          }
        },
        {
          title: 'DevMenu',
        ...
        },
        {
          ...
          onPress: () => {
            ActionManager.hide();
            StorageManager.remove('DevRefresh');
            StorageManager.remove('PersistenceKey');
            DevSettings.reload();
          }
        }
      ]
    };
    __ANDROID__ && params.actions.splice(1,1);
  ...
  useEffect(() => {
    StorageManager.load('DevRefresh').then((response) => {
      ...
      >
        ...
        >
          <Button
            ...
            iconStyle={{ width: 40, height: 40, tintColor: Predefine.themeColor }}
            onPress={onPress}
